...
double find_constant_term(TestCase *tc) {
    int k = tc->keys.k;
    ...
    return x[k - 1];  // Return the constant term (c)
}

int is_point_on_curve(TestCase *tc, int index, double constant_term) {
    int k = tc->keys.k;
    double x = index + 1;
    ...
    for (int i = 0; i < k - 1; i++) {
        calculated_y += pow(x, k - 1 - i) * (y - constant_term) / pow(x, k - 1);
    }
    calculated_y += constant_term;

    return fabs(y - calculated_y) < 1e-6;
}

void solve_test_case(TestCase *tc) {
    double constant_term = find_constant_term(tc);
    printf("Constant term (c): %.0f\n", constant_term);  // Print constant term

    if (tc->keys.n > tc->keys.k) {
        printf("Wrong points: ");
        int wrong_count = 0;
        for (int i = 0; i < tc->keys.n; i++) {
            if (!is_point_on_curve(tc, i, constant_term)) {
                printf("%d ", i + 1);
                wrong_count++;
            }
        }
